...
def get_rest(c_id):
    ...
    for j in range(len(name)):
        d = resto.objects.count()
        if d == 30:
            if resto.objects.filter(name=name[j],img=pic[j],adress=address[j],url=url[j]).exists():
                break
            else:
                ab = resto.objects.update(name=name[j],img=pic[j],adress=address[j],url=url[j])
         
        else:
            ab = resto.objects.create(name=name[j],img=pic[j],adress=address[j],url=url[j])
            ab.save()
